    ...
    <div className="space-y-8 animate-fadeIn pb-24 pr-2">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="flex items-center space-x-6">
           <div className="p-4 bg-accent text-white rounded-3xl shadow-xl"><History size={28} /></div>
           <div>
        ...
        <div className="flex items-center space-x-3">
          <div className="flex bg-white dark:bg-slate-900 p-1.5 rounded-2xl border shadow-sm">
             <button onClick={() => setActiveFilter('all')} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${activeFilter === 'all' ? 'bg-accent text-white' : 'text-slate-400'}`}>Tous</button>
          ...
          <button onClick={startEventReservation} className="bg-accent text-white px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center hover:opacity-90">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800 shadow-sm overflow-hidden flex-1">
        <div className="p-6 border-b border-slate-100 dark:border-slate-800 bg-slate-50/30 flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="relative w-full max-w-md">
             ...
             <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Rechercher (Client, N°...)" className="w-full pl-12 pr-6 py-3 bg-white dark:bg-slate-800 border-none rounded-2xl text-xs font-bold outline-none shadow-sm focus:ring-2 focus:ring-accent" />
          </div>
          <div className="flex items-center space-x-3 bg-white dark:bg-slate-800 p-2 rounded-2xl border shadow-sm">
             <Calendar size={14} className="text-accent ml-2" />
                ...
                <tr key={sale.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-all group">
                  <td className="px-8 py-6">
                    <div className="flex flex-col">
                       <span className="text-xs font-mono font-black text-accent">#{sale.id.slice(-8)}</span>
             ...
             <div className="w-full md:w-1/3 border-r dark:border-slate-800 flex flex-col overflow-hidden bg-slate-50 dark:bg-slate-900/50">
                <div className="p-6 border-b dark:border-slate-800 bg-white dark:bg-slate-800/30">
                   ...
                   <div className="relative">
                      ...
                      <input type="text" placeholder="Rechercher..." className="w-full pl-10 pr-4 py-2 bg-slate-50 dark:bg-slate-900 rounded-xl text-xs border-none shadow-sm focus:ring-2 focus:ring-accent outline-none" />
                   </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 grid grid-cols-1 gap-2 scrollbar-hide">
                   {products.map(p => (
                     <button key={p.id} onClick={() => addItemToSale(p)} className="p-3 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-between hover:border-accent border-2 border-transparent transition-all group text-left shadow-sm">
                        <div className="flex flex-col min-w-0">
                           <span className="text-[10px] font-black uppercase truncate text-slate-700 dark:text-white group-hover:text-accent">{p.name}</span>
                           <span className="text-[8px] font-bold text-slate-400">{p.price} {config.currency}</span>
                        </div>
                        <Plus size={14} className="text-slate-300 group-hover:text-accent shrink-0 ml-2"/>
                     </button>
                   ))}
                </div>
             </div>
             ...
             <div className="flex-1 flex flex-col overflow-hidden bg-white dark:bg-slate-900">
                ...
                <form onSubmit={handleSaveSale} className="flex-1 flex flex-col p-6 space-y-6 overflow-hidden">
                   ...
                   {editingSale.eventDetails && (
                     <div className="p-5 bg-accent/5 dark:bg-accent/10 rounded-3xl border border-accent/10 dark:border-accent/50 space-y-4">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                           <div className="space-y-1">
                              ...
                              <div className="relative">
                                <Calendar size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-accent"/>
                           ...
                           <div className="space-y-1">
                              ...
                              <div className="relative">
                                <Users size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-accent"/>
                           ...
                           <div className="space-y-1">
                              ...
                              <div className="relative">
                                <MapPin size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-accent"/>
                   ...
                   <div className="flex-1 overflow-y-auto space-y-2 pr-2 scrollbar-hide">
                      <p className="text-[9px] font-black uppercase text-slate-400 border-b pb-2 flex items-center justify-between">
                         Lignes de Commande
                         <span className="text-accent">{editingSale.items?.length || 0} Article(s)</span>
                      </p>
                      {editingSale.items?.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border group hover:border-accent/20 transition-colors">
                           ...
                           <div className="flex items-center space-x-3">
                              ...
                              <button type="button" onClick={() => adjustItemQty(item.productId, 1)} className="p-1 bg-white dark:bg-slate-900 rounded-lg shadow-sm hover:text-accent"><Plus size={12}/></button>
                   ...
                   <div className="pt-6 border-t dark:border-slate-800 flex items-center justify-between">
                      <div className="flex flex-col">
                         ...
                         <span className="text-2xl font-black text-accent">{(editingSale.items?.reduce((acc, i) => acc + i.price * i.quantity, 0) || 0).toLocaleString()} <span className="text-xs">{config.currency}</span></span>
                      </div>
                      <div className="flex items-center space-x-3">
                         ...
                         <button type="submit" className="px-10 py-4 bg-accent text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center hover:opacity-90">
